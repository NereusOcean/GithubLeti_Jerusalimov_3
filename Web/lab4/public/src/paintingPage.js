...
export function renderLib(lib){
    ...
    for(let i =0; i < lib.length; i++){
        ...
        boxPainting.last().find(`div.textBox`).last().append(`<text id="currentCost">Текущая ценна:  ${lib[i]["currentCost"]}</text>`);
        boxPainting.last().find(`div.textBox`).last().append(`<text id="sellFor">Продано за:  ${lib[i]["sellFor"]}</text>`);
        boxPainting.last().find(`div.textBox`).last().append(`<text id="buyer">покупатель:  ${lib[i]["buyer"]}</text>`);
